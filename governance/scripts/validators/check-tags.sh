...
check_resource_tags() {
    ...
    local skip_types=("aws_kms_alias" "aws_kms_key_policy" "aws_ecr_repository_policy" "aws_ecr_lifecycle_policy" "aws_iam_role_policy_attachment" "aws_iam_role_policy" "aws_iam_access_key" "aws_iam_user_policy_attachment" "aws_ecs_cluster_capacity_providers" "aws_secretsmanager_secret_version" "aws_secretsmanager_secret_rotation" "aws_lambda_permission" "aws_lambda_alias" "aws_lambda_function_url" "aws_lambda_event_source_mapping" "aws_lambda_layer_version" "aws_lambda_provisioned_concurrency_config" "aws_efs_mount_target" "aws_efs_access_point" "aws_route" "aws_route_table_association" "aws_security_group_rule" "aws_vpc_endpoint_route_table_association" "aws_lb_target_group_attachment" "aws_lb_listener" "aws_lb_listener_rule" "aws_sns_topic_policy" "aws_sns_topic_subscription" "aws_sqs_queue_policy" "aws_cloudwatch_event_target" "aws_cloudwatch_log_metric_filter" "aws_cloudwatch_log_subscription_filter" "aws_cloudwatch_log_stream" "aws_grafana_role_association" "aws_appautoscaling_target" "aws_appautoscaling_policy" "aws_appautoscaling_scheduled_action" "aws_route53_record" "aws_route53_query_log" "aws_acm_certificate_validation" "aws_wafv2_web_acl_logging_configuration" "aws_wafv2_web_acl_association" "aws_glue_catalog_table" "aws_athena_named_query" "aws_cloudfront_origin_access_control" "aws_cloudfront_function" "aws_cloudfront_cache_policy" "aws_cloudfront_origin_request_policy" "aws_cloudfront_response_headers_policy" "aws_db_proxy_default_target_group" "aws_db_proxy_target" "aws_kinesis_stream_consumer" "null_resource" "terraform_data" "time_sleep" "data")
    ...
    # Check if using merge(local.required_tags) or merge(var.common_tags) or merge(module.common_tags.tags) or merge(local.common_tags) pattern
    # Also accept direct usage: tags = module.common_tags.tags or tags = var.common_tags or tags = local.required_tags or tags = local.common_tags
    # Convert to single line for pattern matching
    if echo "$resource_block" | tr '\n' ' ' | grep -qE "(merge.*(local\.required_tags|local\.common_tags|var\.common_tags|module\.common_tags\.tags)|tags\s*=\s*(module\.common_tags\.tags|var\.common_tags|local\.required_tags|local\.common_tags))"; then

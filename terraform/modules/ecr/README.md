...
- **암호화**: KMS 또는 AES256 암호화 지원 (KMS 권장, AES256은 기존 리소스 호환용)
...
| 이름 | 타입 | 기본값 | 설명 | 제약사항 |
|------|------|--------|------|----------|
| `encryption_type` | string | "KMS" | 암호화 유형 | AES256 또는 KMS |
| `kms_key_arn` | string | null | KMS 키 ARN | encryption_type이 "KMS"인 경우 필수 |

> **참고**: `encryption_type`이 "KMS"인 경우 `kms_key_arn`을 반드시 지정해야 합니다. AES256은 기존 ECR 리포지토리와의 호환성을 위해 지원됩니다. 신규 리포지토리에는 KMS 암호화를 권장합니다.
...
### 암호화 정책
- **신규 리포지토리**: 고객 관리형 KMS 키 사용 권장
- **기존 리포지토리**: AES256 암호화 허용 (레거시 호환성)
- KMS 사용 시 키는 자동 로테이션이 활성화되어야 합니다
- 정책 예외 사항은 [ecr-aes256.md](../../../../claudedocs/policy-exceptions/ecr-aes256.md) 참조

...
resource "aws_glue_catalog_table" "cloudtrail" {
  ...
  parameters = {
    ...
    "storage.location.template" = "s3://${aws_s3_bucket.cloudtrail.id}/${var.s3_key_prefix}/AWSLogs/${var.aws_account_id}/CloudTrail/$${region}/$${date}"
  }

  storage_descriptor {
    location      = "s3://${aws_s3_bucket.cloudtrail.id}/${var.s3_key_prefix}/AWSLogs/${var.aws_account_id}/CloudTrail/"

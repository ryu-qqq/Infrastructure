# Infrastructure Shared Resources Command

**Task**: infrastructure 프로젝트에서 이미 배포된 공유 인프라를 다른 프로젝트에서 참조할 수 있게 도와줍니다.

## 핵심 개념

이 infrastructure 레포는 **중앙 집중식 공유 인프라**를 관리합니다.
다른 프로젝트(예: fileflow, api-server)에서 이미 배포된 공유 리소스를 SSM Parameter Store를 통해 참조할 수 있습니다.

**참조 방법**: SSM Parameter Store + Terraform data sources (import 불필요)

## 모듈 vs 공유 인프라

| 구분 | 모듈 (`/if/module`) | 공유 인프라 (`/if/shared`) |
|------|-------------------|------------------------|
| 목적 | 새 리소스 생성 | 기존 리소스 참조 |
| 방식 | Terraform module 사용 | Terraform data source 사용 |
| 버전 | Git 태그 버전 관리 | 배포된 현재 상태 |
| 예시 | ECR, ALB, ECS 생성 | RDS, VPC, S3 참조 |

## 사용 가능한 커맨드

### 1. 📋 공유 리소스 목록 조회

```bash
# Claude Code에서
/if/shared list

# 또는 직접 실행
./scripts/shared/shared-infra-manager.sh list
```

**출력 예시**:
```
ℹ️  사용 가능한 공유 인프라 리소스:

🗄️  tfstate
   📝 Terraform State Storage (S3)
   📂 terraform/tfstate

🔒 tflock
   📝 Terraform State Lock (DynamoDB)
   📂 terraform/tfstate

🌐 vpc
   📝 VPC Network
   📂 terraform/network

📊 rds
   📝 RDS MySQL Database
   📂 terraform/rds

🔑 kms
   📝 KMS Encryption Keys
   📂 terraform/kms

📈 amp
   📝 Amazon Managed Prometheus
   📂 terraform/monitoring

📊 amg
   📝 Amazon Managed Grafana
   📂 terraform/monitoring

🌍 route53
   📝 Route53 Hosted Zones
   📂 terraform/dns

🔒 acm
   📝 ACM SSL Certificates
   📂 terraform/dns

🔐 secrets
   📝 Secrets Manager
   📂 terraform/secrets

📋 cloudtrail
   📝 CloudTrail Audit Logging
   📂 terraform/audit

📝 logging
   📝 Centralized Logging (S3)
   📂 terraform/logging
```

### 2. 🔍 공유 리소스 상세 정보

```bash
# Claude Code에서
/if/shared info rds

# 또는 직접 실행
./scripts/shared/shared-infra-manager.sh info rds
```

**출력 내용**:
- SSM Parameter Store 경로 목록
- 현재 AWS에 배포된 실제 값 (AWS CLI 사용 가능 시)
- Terraform data source 사용 예제
- Secrets Manager 통합 (RDS 등)

**출력 예시**:
```
ℹ️  공유 리소스: RDS MySQL Database

📋 SSM Parameters:
  ✓ /shared/rds/db-instance-id
    Current value: prod-shared-mysql

  ✓ /shared/rds/db-instance-address
    Current value: prod-shared-mysql.cfacertspqbw.ap-northeast-2.rds.amazonaws.com

  ✓ /shared/rds/db-instance-port
    Current value: 3306

  ✓ /shared/rds/master-password-secret-name
    Current value: prod-shared-mysql-master-password

  ✓ /shared/rds/db-security-group-id
    Current value: sg-0d9b6f65239b16b44

💡 Terraform에서 사용하는 방법:
data "aws_ssm_parameter" "rds_address" {
  name = "/shared/rds/db-instance-address"
}

output "rds_endpoint" {
  value = data.aws_ssm_parameter.rds_address.value
}
```

### 3. 📦 공유 리소스 코드 생성

```bash
# Claude Code에서
/if/shared get rds

# 또는 직접 실행
./scripts/shared/shared-infra-manager.sh get rds
```

**출력 내용**:
- 완전한 Terraform data source 코드
- SSM Parameter 참조
- Secrets Manager 통합 (필요 시)
- Local variables로 디코딩된 값

**생성되는 파일 예시 (`/tmp/shared-rds.tf`)**:
```hcl
# Shared Infrastructure Reference: RDS MySQL Database
# Auto-generated by /if/shared get rds
# Description: Shared MySQL database

# RDS Connection Information
data "aws_ssm_parameter" "rds_address" {
  name = "/shared/rds/db-instance-address"
}

data "aws_ssm_parameter" "rds_port" {
  name = "/shared/rds/db-instance-port"
}

data "aws_ssm_parameter" "rds_security_group" {
  name = "/shared/rds/db-security-group-id"
}

# RDS Credentials (from Secrets Manager)
data "aws_ssm_parameter" "rds_secret_name" {
  name = "/shared/rds/master-password-secret-name"
}

data "aws_secretsmanager_secret" "rds" {
  name = data.aws_ssm_parameter.rds_secret_name.value
}

data "aws_secretsmanager_secret_version" "rds" {
  secret_id = data.aws_secretsmanager_secret.rds.id
}

# Decoded credentials
locals {
  rds_credentials = jsondecode(data.aws_secretsmanager_secret_version.rds.secret_string)
  rds_username    = local.rds_credentials.username
  rds_password    = local.rds_credentials.password
  rds_dbname      = local.rds_credentials.dbname
  rds_endpoint    = "${data.aws_ssm_parameter.rds_address.value}:${data.aws_ssm_parameter.rds_port.value}"
}
```

## 워크플로우 예시

### FileFlow 프로젝트에서 공유 RDS 사용하기

```bash
# 1. FileFlow 프로젝트로 이동
cd ~/fileflow

# 2. 사용 가능한 공유 리소스 확인
/if/shared list

# 3. RDS 상세 정보 확인
/if/shared info rds

# 4. RDS 참조 코드 생성
/if/shared get rds

# 5. 생성된 파일을 프로젝트로 복사
cp /tmp/shared-rds.tf terraform/shared-rds.tf

# 6. 애플리케이션 코드에서 사용
# main.tf에서 locals를 참조
resource "aws_ecs_task_definition" "app" {
  # ...

  environment = [
    {
      name  = "DB_HOST"
      value = local.rds_endpoint
    },
    {
      name  = "DB_USER"
      value = local.rds_username
    }
  ]

  secrets = [
    {
      name      = "DB_PASSWORD"
      valueFrom = data.aws_secretsmanager_secret.rds.arn
    }
  ]
}
```

### 여러 공유 리소스를 함께 사용하는 경우

```bash
# 1. 필요한 공유 리소스 코드 생성
/if/shared get vpc
/if/shared get rds
/if/shared get kms
/if/shared get amp

# 2. 프로젝트로 복사
cp /tmp/shared-vpc.tf terraform/
cp /tmp/shared-rds.tf terraform/
cp /tmp/shared-kms.tf terraform/
cp /tmp/shared-amp.tf terraform/

# 3. 프로젝트 구조
fileflow/
├── src/                    # 애플리케이션 코드
├── terraform/
│   ├── provider.tf
│   ├── variables.tf
│   ├── shared-vpc.tf       # VPC 참조
│   ├── shared-rds.tf       # RDS 참조
│   ├── shared-kms.tf       # KMS 참조
│   ├── shared-amp.tf       # AMP 참조
│   └── main.tf             # 애플리케이션 리소스
└── atlantis.yaml
```

## 사용 가능한 공유 리소스

### 🗄️ Terraform State (tfstate)
**SSM Parameters**:
- `/shared/tfstate/bucket-name`: S3 버킷 이름
- `/shared/tfstate/bucket-arn`: S3 버킷 ARN
- `/shared/tfstate/kms-key-id`: KMS 키 ID

**사용 목적**: Terraform backend 설정

```hcl
terraform {
  backend "s3" {
    bucket         = "ryuqqq-prod-tfstate"
    key            = "fileflow/terraform.tfstate"
    region         = "ap-northeast-2"
    encrypt        = true
    dynamodb_table = "terraform-lock"
    kms_key_id     = "alias/terraform-state"
  }
}
```

### 🔒 Terraform Lock (tflock)
**SSM Parameters**:
- `/shared/tflock/table-name`: DynamoDB 테이블 이름
- `/shared/tflock/table-arn`: DynamoDB 테이블 ARN

**사용 목적**: Terraform state lock

### 🌐 VPC (vpc)
**SSM Parameters**:
- `/shared/vpc/vpc-id`: VPC ID
- `/shared/vpc/cidr-block`: VPC CIDR

**사용 목적**: 보안 그룹, 서브넷 설정

```hcl
resource "aws_security_group" "app" {
  vpc_id = data.aws_ssm_parameter.vpc_id.value
  # ...
}
```

### 📡 Subnets (subnets)
**SSM Parameters**:
- `/shared/vpc/public-subnet-ids`: Public 서브넷 (쉼표 구분)
- `/shared/vpc/private-subnet-ids`: Private 서브넷 (쉼표 구분)
- `/shared/vpc/data-subnet-ids`: Data 서브넷 (쉼표 구분)

**사용 목적**: ALB, ECS, RDS 배치

```hcl
resource "aws_lb" "main" {
  subnets = split(",", data.aws_ssm_parameter.public_subnet_ids.value)
  # ...
}
```

### 📊 RDS (rds)
**SSM Parameters**:
- `/shared/rds/db-instance-id`: RDS 인스턴스 ID
- `/shared/rds/db-instance-address`: RDS 엔드포인트
- `/shared/rds/db-instance-port`: RDS 포트
- `/shared/rds/master-password-secret-name`: Secrets Manager 이름
- `/shared/rds/db-security-group-id`: RDS 보안 그룹

**사용 목적**: 공유 MySQL 데이터베이스 연결

### 🔑 KMS (kms)
**SSM Parameters**:
- `/shared/kms/ebs-key-arn`: EBS 암호화 키
- `/shared/kms/s3-key-arn`: S3 암호화 키
- `/shared/kms/rds-key-arn`: RDS 암호화 키
- `/shared/kms/secrets-key-arn`: Secrets Manager 키
- `/shared/kms/logs-key-arn`: CloudWatch Logs 키

**사용 목적**: 리소스 암호화

```hcl
resource "aws_s3_bucket_server_side_encryption_configuration" "example" {
  rule {
    apply_server_side_encryption_by_default {
      sse_algorithm     = "aws:kms"
      kms_master_key_id = data.aws_ssm_parameter.s3_kms_key.value
    }
  }
}
```

### 📈 AMP (Amazon Managed Prometheus)
**SSM Parameters**:
- `/shared/monitoring/amp-workspace-id`: AMP Workspace ID
- `/shared/monitoring/amp-endpoint`: Prometheus 엔드포인트

**사용 목적**: 메트릭 수집 및 모니터링

```hcl
# Prometheus remote write 설정
remote_write:
  - url: "${data.aws_ssm_parameter.amp_endpoint.value}api/v1/remote_write"
```

### 📊 AMG (Amazon Managed Grafana)
**SSM Parameters**:
- `/shared/monitoring/amg-workspace-id`: AMG Workspace ID
- `/shared/monitoring/amg-endpoint`: Grafana 엔드포인트

**사용 목적**: 대시보드 및 시각화

### 🌍 Route53 (route53)
**SSM Parameters**:
- `/shared/dns/hosted-zone-id`: Route53 Hosted Zone ID
- `/shared/dns/domain-name`: 도메인 이름

**사용 목적**: DNS 레코드 생성

```hcl
resource "aws_route53_record" "app" {
  zone_id = data.aws_ssm_parameter.hosted_zone_id.value
  name    = "app.${data.aws_ssm_parameter.domain_name.value}"
  # ...
}
```

### 🔒 ACM (acm)
**SSM Parameters**:
- `/shared/acm/certificate-arn`: SSL 인증서 ARN

**사용 목적**: ALB HTTPS 리스너

```hcl
resource "aws_lb_listener" "https" {
  certificate_arn = data.aws_ssm_parameter.certificate_arn.value
  # ...
}
```

### 🔐 Secrets Manager (secrets)
**SSM Parameters**:
- `/shared/secrets/kms-key-arn`: Secrets 암호화 키

**사용 목적**: 애플리케이션 시크릿 저장

### 📋 CloudTrail (cloudtrail)
**SSM Parameters**:
- `/shared/audit/cloudtrail-name`: CloudTrail 이름
- `/shared/audit/cloudtrail-arn`: CloudTrail ARN
- `/shared/audit/cloudtrail-bucket`: 감사 로그 S3 버킷

**사용 목적**: 감사 로그 참조

### 📝 Logging (logging)
**SSM Parameters**:
- `/shared/logging/s3-bucket`: 중앙 로그 S3 버킷
- `/shared/logging/firehose-arn`: Kinesis Firehose ARN

**사용 목적**: 애플리케이션 로그 전송

## 자동화 스크립트

**위치**: `/path/to/infrastructure/scripts/shared/shared-infra-manager.sh`

**직접 실행**:
```bash
# Infrastructure 레포에서
./scripts/shared/shared-infra-manager.sh list
./scripts/shared/shared-infra-manager.sh info rds
./scripts/shared/shared-infra-manager.sh get rds

# 다른 프로젝트에서 (절대 경로 사용)
/path/to/infrastructure/scripts/shared/shared-infra-manager.sh list
```

## 주의사항

### ✅ Do's

- 공유 인프라는 **읽기 전용**으로 참조만 하세요
- SSM Parameter Store 값이 변경될 수 있으므로 하드코딩하지 마세요
- Secrets Manager를 통해 안전하게 크레덴셜 접근하세요
- 각 프로젝트에서 필요한 리소스만 참조하세요

### ❌ Don'ts

- 공유 인프라를 직접 수정하지 마세요 (Infrastructure 레포에서만 관리)
- `terraform import`로 가져오지 마세요 (data source 사용)
- SSM Parameter 이름을 하드코딩하지 마세요
- 크레덴셜을 환경 변수나 코드에 직접 저장하지 마세요

## 트러블슈팅

### SSM Parameter를 찾을 수 없음

```bash
# 증상: Parameter not found
Error: error getting SSM parameter: ParameterNotFound

# 해결:
1. Infrastructure 레포에서 해당 리소스가 배포되었는지 확인
2. SSM Parameter가 올바른 경로에 export되었는지 확인
3. AWS 리전이 올바른지 확인 (ap-northeast-2)
```

### Secrets Manager 접근 권한 없음

```bash
# 증상: Access denied to Secrets Manager
Error: AccessDeniedException

# 해결:
1. IAM 역할에 Secrets Manager 읽기 권한 추가
2. KMS 키에 대한 복호화 권한 확인
3. Resource-based policy 확인
```

### 값이 업데이트되지 않음

```bash
# 증상: Terraform이 이전 값을 계속 사용
Plan shows no changes

# 해결:
1. terraform refresh 실행
2. SSM Parameter Store에서 실제 값 확인 (AWS Console or CLI)
3. Data source가 올바른 parameter를 참조하는지 확인
```

## 관련 문서

- [Multi-Repo 아키텍처 전략](../../architecture/multi-repo-strategy.md)
- [SSM Parameter Store 가이드](../../guides/ssm-parameters.md)
- [Module 개발 가이드](./module.md)
- [Atlantis 설정 가이드](./atlantis.md)

## 실전 예제

### 예제 1: FileFlow에서 공유 RDS + VPC 사용

```hcl
# terraform/shared-infrastructure.tf
# RDS 참조
data "aws_ssm_parameter" "rds_address" {
  name = "/shared/rds/db-instance-address"
}

data "aws_ssm_parameter" "rds_secret_name" {
  name = "/shared/rds/master-password-secret-name"
}

data "aws_secretsmanager_secret_version" "rds" {
  secret_id = data.aws_secretsmanager_secret.rds.id
}

# VPC 참조
data "aws_ssm_parameter" "vpc_id" {
  name = "/shared/vpc/vpc-id"
}

data "aws_ssm_parameter" "private_subnet_ids" {
  name = "/shared/vpc/private-subnet-ids"
}

locals {
  rds_credentials = jsondecode(data.aws_secretsmanager_secret_version.rds.secret_string)
  vpc_id          = data.aws_ssm_parameter.vpc_id.value
  private_subnets = split(",", data.aws_ssm_parameter.private_subnet_ids.value)
}

# terraform/main.tf
resource "aws_ecs_task_definition" "fileflow" {
  family = "fileflow"

  container_definitions = jsonencode([
    {
      name  = "fileflow"
      image = "${aws_ecr_repository.fileflow.repository_url}:latest"

      environment = [
        {
          name  = "DB_HOST"
          value = data.aws_ssm_parameter.rds_address.value
        },
        {
          name  = "DB_USER"
          value = local.rds_credentials.username
        },
        {
          name  = "DB_NAME"
          value = local.rds_credentials.dbname
        }
      ]

      secrets = [
        {
          name      = "DB_PASSWORD"
          valueFrom = data.aws_secretsmanager_secret.rds.arn
        }
      ]
    }
  ])
}

resource "aws_security_group" "fileflow" {
  vpc_id = local.vpc_id

  egress {
    from_port   = 3306
    to_port     = 3306
    protocol    = "tcp"
    cidr_blocks = ["10.0.0.0/16"]
  }
}
```

### 예제 2: API Server에서 공유 모니터링 스택 사용

```hcl
# terraform/shared-monitoring.tf
data "aws_ssm_parameter" "amp_endpoint" {
  name = "/shared/monitoring/amp-endpoint"
}

data "aws_ssm_parameter" "amg_endpoint" {
  name = "/shared/monitoring/amg-endpoint"
}

# Prometheus remote write 설정
resource "aws_ecs_task_definition" "api_server" {
  container_definitions = jsonencode([
    {
      name  = "api-server"

      environment = [
        {
          name  = "PROMETHEUS_REMOTE_WRITE_URL"
          value = "${data.aws_ssm_parameter.amp_endpoint.value}api/v1/remote_write"
        }
      ]
    }
  ])
}
```

### 예제 3: 완전한 공유 인프라 참조 패턴

```hcl
# terraform/shared.tf - 모든 공유 리소스 한 번에 참조
locals {
  # VPC
  vpc_id          = data.aws_ssm_parameter.vpc_id.value
  public_subnets  = split(",", data.aws_ssm_parameter.public_subnet_ids.value)
  private_subnets = split(",", data.aws_ssm_parameter.private_subnet_ids.value)

  # RDS
  rds_endpoint    = data.aws_ssm_parameter.rds_address.value
  rds_credentials = jsondecode(data.aws_secretsmanager_secret_version.rds.secret_string)

  # KMS
  kms_key_s3   = data.aws_ssm_parameter.s3_kms_key.value
  kms_key_logs = data.aws_ssm_parameter.logs_kms_key.value

  # DNS
  hosted_zone_id  = data.aws_ssm_parameter.hosted_zone_id.value
  domain_name     = data.aws_ssm_parameter.domain_name.value
  certificate_arn = data.aws_ssm_parameter.certificate_arn.value

  # Monitoring
  amp_endpoint = data.aws_ssm_parameter.amp_endpoint.value
  amg_endpoint = data.aws_ssm_parameter.amg_endpoint.value
}

# terraform/main.tf에서 local 값들을 사용
resource "aws_lb" "main" {
  subnets            = local.public_subnets
  security_groups    = [aws_security_group.alb.id]

  tags = {
    Name = "${var.service_name}-alb"
  }
}

resource "aws_lb_listener" "https" {
  load_balancer_arn = aws_lb.main.arn
  port              = 443
  protocol          = "HTTPS"
  certificate_arn   = local.certificate_arn
}

resource "aws_route53_record" "app" {
  zone_id = local.hosted_zone_id
  name    = "${var.service_name}.${local.domain_name}"
  type    = "A"

  alias {
    name                   = aws_lb.main.dns_name
    zone_id                = aws_lb.main.zone_id
    evaluate_target_health = true
  }
}
```

## FAQ

**Q: 공유 인프라와 모듈의 차이는 무엇인가요?**
A: 모듈은 새 리소스를 만드는 템플릿이고, 공유 인프라는 이미 배포된 리소스를 참조하는 것입니다.

**Q: SSM Parameter 값이 변경되면 어떻게 하나요?**
A: `terraform refresh` 또는 `terraform plan`을 실행하면 자동으로 최신 값을 가져옵니다.

**Q: 공유 인프라를 수정하고 싶어요.**
A: Infrastructure 레포의 해당 Terraform 코드를 수정하고 배포해야 합니다. 다른 프로젝트에서는 읽기 전용입니다.

**Q: 모든 공유 리소스를 반드시 사용해야 하나요?**
A: 아니요, 프로젝트에 필요한 리소스만 참조하세요.

**Q: Private repo에서도 작동하나요?**
A: 네, SSM Parameter Store와 Secrets Manager는 AWS 계정 내에서 작동하므로 repo 접근 권한과 무관합니다.

**Q: 여러 환경(dev/staging/prod)에서 다른 공유 인프라를 참조할 수 있나요?**
A: 네, SSM Parameter 경로에 환경 접두사를 추가하면 됩니다 (예: `/dev/shared/rds/...`).

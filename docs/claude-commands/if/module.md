# Infrastructure Module Command

**Task**: infrastructure 프로젝트의 Terraform 모듈을 다른 프로젝트에서 쉽게 가져다 쓸 수 있게 도와줍니다.

## 핵심 개념

이 infrastructure 레포는 **재사용 가능한 Terraform 모듈**을 제공합니다.
다른 프로젝트(예: fileflow, api-server)에서 이 모듈들을 Git source로 참조하여 인프라를 구성할 수 있습니다.

**버전 관리**: Git 태그로 모듈 버전 관리 (`modules/{module-name}/v{version}`)

## 사용 가능한 커맨드

### 1. 📋 모듈 목록 조회

```bash
# Claude Code에서
/if/module list

# 또는 직접 실행
./scripts/modules/module-manager.sh list
```

**출력 예시**:
```
ℹ️  사용 가능한 Infrastructure 모듈 목록:

📦 alb
   Latest: v1.0.0
   📝 Application Load Balancer with HTTPS support

📦 ecr
   Latest: v1.0.0 (CHANGELOG only)
   📝 Amazon ECR repository with KMS encryption

📦 ecs-service
   Latest: v1.2.0
   All versions: v1.0.0 v1.1.0 v1.2.0
   📝 ECS Service with Auto Scaling
```

### 2. 🔍 모듈 상세 정보

```bash
# Claude Code에서
/if/module info ecr

# 또는 직접 실행
./scripts/modules/module-manager.sh info ecr
```

**출력 내용**:
- 모듈 버전 정보
- README 내용
- 디렉토리 구조
- 기본 사용 예제

### 3. 📦 모듈 가져오기 (Terraform source 생성)

```bash
# 최신 버전 가져오기
/if/module get ecr

# 특정 버전 가져오기
/if/module get ecr@v1.0.0

# 또는 직접 실행
./scripts/modules/module-manager.sh get ecr@v1.0.0
```

**출력 예시**:
```
✅ Module: ecr (v1.0.0)

Terraform에서 사용하는 방법:

module "ecr" {
  source = "git::https://github.com/ryuqqq/infrastructure.git//terraform/modules/ecr?ref=modules/ecr/v1.0.0"

  # 여기에 모듈 변수를 추가하세요
}

사용 가능한 변수:
  - repository_name
  - image_tag_mutability
  - scan_on_push
  - kms_key_arn
  - lifecycle_policy
  - common_tags

ℹ️  Tip: /if/module init ecr 를 실행하면 자동으로 설정 파일이 생성됩니다
```

### 4. 🚀 모듈 초기화 (현재 프로젝트에 설정 파일 생성)

```bash
# 최신 버전으로 초기화
/if/module init ecr

# 특정 버전으로 초기화
/if/module init alb@v1.0.0

# 또는 직접 실행
./scripts/modules/module-manager.sh init ecr
```

**생성되는 파일들**:
```
terraform/
├── provider.tf                    # 생성됨 (없는 경우)
├── variables.tf                   # 생성됨 (없는 경우)
└── ecr/                          # 새로 생성
    ├── main.tf                   # 모듈 참조 코드
    ├── outputs.tf                # 출력 변수
    ├── example.tf.template       # 사용 예제 (참고용)
    └── variables-reference.tf.md # 변수 문서
```

**main.tf 예시**:
```hcl
# ecr Module Configuration
# Version: v1.0.0
# Auto-generated by /if/module init

module "ecr" {
  source = "git::https://github.com/ryuqqq/infrastructure.git//terraform/modules/ecr?ref=modules/ecr/v1.0.0"

  # TODO: Configure module variables
  # See variables.tf for available options
}
```

## 워크플로우 예시

### FileFlow 프로젝트에서 ECR 모듈 사용하기

```bash
# 1. FileFlow 프로젝트로 이동
cd ~/fileflow

# 2. 사용 가능한 모듈 확인
/if/module list

# 3. ECR 모듈 상세 정보 확인
/if/module info ecr

# 4. ECR 모듈 초기화 (설정 파일 자동 생성)
/if/module init ecr

# 5. 생성된 main.tf 편집
vim terraform/ecr/main.tf

# 6. Terraform 실행
cd terraform/ecr
terraform init
terraform plan
terraform apply
```

### 여러 모듈을 사용하는 경우

```bash
# 1. 필요한 모듈들 초기화
/if/module init ecr
/if/module init alb
/if/module init ecs-service

# 2. 각 모듈 설정
# terraform/ecr/main.tf
# terraform/alb/main.tf
# terraform/ecs-service/main.tf

# 3. 프로젝트 구조
fileflow/
├── src/                    # 애플리케이션 코드
├── terraform/
│   ├── provider.tf
│   ├── variables.tf
│   ├── ecr/
│   │   ├── main.tf
│   │   └── outputs.tf
│   ├── alb/
│   │   ├── main.tf
│   │   └── outputs.tf
│   └── ecs-service/
│       ├── main.tf
│       └── outputs.tf
└── atlantis.yaml
```

## 버전 관리 전략

### Git 태그 형식
```
modules/{module-name}/v{major}.{minor}.{patch}

예시:
modules/ecr/v1.0.0
modules/alb/v1.1.0
modules/ecs-service/v2.0.0
```

### Semantic Versioning

- **Major (v2.0.0)**: Breaking changes (호환성 깨짐)
  - 필수 변수 추가
  - 출력 변수 삭제
  - 리소스 이름 변경

- **Minor (v1.1.0)**: 기능 추가 (하위 호환)
  - 선택적 변수 추가
  - 새 출력 변수 추가
  - 새 기능 추가

- **Patch (v1.0.1)**: 버그 수정 (하위 호환)
  - 버그 수정
  - 문서 개선
  - 리팩토링

### 버전 업그레이드

```bash
# 1. 현재 사용 중인 버전 확인
grep "ref=" terraform/ecr/main.tf

# 2. 새 버전 확인
/if/module info ecr

# 3. 새 버전으로 업데이트
# terraform/ecr/main.tf에서 ref 값 변경
source = "git::...?ref=modules/ecr/v1.1.0"

# 4. Terraform 재초기화
cd terraform/ecr
terraform init -upgrade
terraform plan
```

## 모듈 개발 가이드

### 새 모듈 만들기 (Infrastructure 레포에서)

```bash
# 1. Infrastructure 레포에서 작업
cd /path/to/infrastructure

# 2. 모듈 디렉토리 생성
mkdir -p terraform/modules/my-module/{examples/basic}

# 3. 필수 파일 생성
cd terraform/modules/my-module
touch main.tf variables.tf outputs.tf versions.tf README.md CHANGELOG.md

# 4. 예제 작성
cd examples/basic
touch main.tf variables.tf
```

### 모듈 버전 태깅

```bash
# 1. CHANGELOG.md 업데이트
vim terraform/modules/ecr/CHANGELOG.md

# 2. 변경사항 커밋
git add terraform/modules/ecr/
git commit -m "feat(ecr): Add lifecycle policy configuration"

# 3. Git 태그 생성
git tag -a modules/ecr/v1.1.0 -m "Release ECR module v1.1.0"

# 4. 푸시
git push origin main
git push origin modules/ecr/v1.1.0
```

## 자동화 스크립트

**위치**: `/path/to/infrastructure/scripts/modules/module-manager.sh`

**직접 실행**:
```bash
# Infrastructure 레포에서
./scripts/modules/module-manager.sh list
./scripts/modules/module-manager.sh info ecr
./scripts/modules/module-manager.sh get ecr@v1.0.0
./scripts/modules/module-manager.sh init ecr

# 다른 프로젝트에서 (절대 경로 사용)
/path/to/infrastructure/scripts/modules/module-manager.sh list
```

## 주의사항

### ✅ Do's

- 항상 버전을 명시해서 사용 (`@v1.0.0`)
- CHANGELOG.md를 반드시 업데이트
- Breaking changes는 Major 버전 업
- 모듈 변경 전 `/if/validate` 실행

### ❌ Don'ts

- `ref=main` 사용 지양 (프로덕션에서)
- 버전 없이 사용하지 말 것
- 모듈 내부 구조를 직접 수정하지 말 것
- Git 태그 삭제하지 말 것

## 트러블슈팅

### 모듈을 찾을 수 없음

```bash
# 증상: Module not found
Error: Failed to download module

# 해결:
1. Git 저장소 URL 확인
2. Git 태그 존재 확인: git tag -l "modules/ecr/*"
3. 네트워크 연결 확인
```

### 버전 충돌

```bash
# 증상: Version conflict
Error: Module version X is not compatible

# 해결:
1. CHANGELOG.md에서 호환성 확인
2. Breaking changes 목록 확인
3. 필요시 코드 마이그레이션
```

### Terraform init 실패

```bash
# 증상: Terraform init fails
Error: Error downloading modules

# 해결:
1. Git credentials 확인
2. SSH 키 설정 확인 (private repo인 경우)
3. Terraform version 확인 (>= 1.0)
```

## 관련 문서

- [Multi-Repo 아키텍처 전략](../../architecture/multi-repo-strategy.md)
- [Module 개발 가이드](../../modules/README.md)
- [Atlantis 설정 가이드](./atlantis.md)

## 실전 예제

### 예제 1: FileFlow ECR 설정

```hcl
# terraform/ecr/main.tf
module "ecr" {
  source = "git::https://github.com/ryuqqq/infrastructure.git//terraform/modules/ecr?ref=modules/ecr/v1.0.0"

  repository_name      = "fileflow"
  image_tag_mutability = "MUTABLE"
  scan_on_push         = true

  lifecycle_policy = {
    rules = [
      {
        rulePriority = 1
        description  = "Keep last 30 tagged images"
        selection = {
          tagStatus     = "tagged"
          tagPrefixList = ["v"]
          countType     = "imageCountMoreThan"
          countNumber   = 30
        }
      }
    ]
  }

  common_tags = {
    Environment = "prod"
    Service     = "fileflow"
    ManagedBy   = "terraform"
  }
}
```

### 예제 2: ALB + ECS Service

```hcl
# terraform/alb/main.tf
module "alb" {
  source = "git::https://github.com/ryuqqq/infrastructure.git//terraform/modules/alb?ref=modules/alb/v1.0.0"

  name               = "fileflow-alb"
  vpc_id             = data.aws_ssm_parameter.vpc_id.value
  subnets            = split(",", data.aws_ssm_parameter.public_subnet_ids.value)
  security_group_ids = [aws_security_group.alb.id]

  certificate_arn = data.aws_ssm_parameter.certificate_arn.value

  common_tags = local.common_tags
}

# terraform/ecs-service/main.tf
module "ecs_service" {
  source = "git::https://github.com/ryuqqq/infrastructure.git//terraform/modules/ecs-service?ref=modules/ecs-service/v1.2.0"

  cluster_id      = data.aws_ecs_cluster.main.id
  service_name    = "fileflow"
  task_definition = aws_ecs_task_definition.fileflow.arn
  desired_count   = 2

  load_balancer = {
    target_group_arn = module.alb.target_group_arn
    container_name   = "fileflow"
    container_port   = 8080
  }

  common_tags = local.common_tags
}
```

## FAQ

**Q: 모듈 버전은 어떻게 선택하나요?**
A: 프로덕션에서는 항상 특정 버전(`v1.0.0`)을 사용하세요. 개발/테스트 환경에서만 `main` 브랜치 사용을 고려하세요.

**Q: 모듈을 수정하고 싶어요.**
A: Infrastructure 레포에서 수정 → Git 태그 생성 → 다른 프로젝트에서 새 버전 참조

**Q: Private repo에서 모듈을 가져올 수 있나요?**
A: 네, SSH 키 또는 Git credentials가 설정되어 있어야 합니다.

**Q: 여러 프로젝트에서 같은 모듈의 다른 버전을 사용할 수 있나요?**
A: 네, 각 프로젝트는 독립적으로 버전을 관리합니다.

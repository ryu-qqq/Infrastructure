# prod-shared-mysql 데이터베이스 접속 가이드
...
Password: !HGLCrllNB1X:A+pBsL7Z!STMc#-7:BL
```
...
DB_NAME=luxury
DB_USER=luxury_user
DB_PASSWORD='!HGLCrllNB1X:A+pBsL7Z!STMc#-7:BL'
```

**연결 문자열 예시**:
```
# MySQL URL
mysql://luxury_user:!HGLCrllNB1X:A+pBsL7Z!STMc#-7:BL@prod-shared-mysql.cfacertspqbw.ap-northeast-2.rds.amazonaws.com:3306/luxury
```

**⚠️ 주의**: DMS를 통해 기존 EC2 MySQL(10.0.142.202)에서 데이터 마이그레이션 진행 중입니다.
...
**접근 방식**:
- 각 프로젝트 사용자(fileflow_user, luxury_user, crawler_user)는 `common` 데이터베이스에 SELECT, INSERT, UPDATE 권한을 가집니다.
...
### 5️⃣ 읽기 전용 계정 (Monitoring/Analytics)
